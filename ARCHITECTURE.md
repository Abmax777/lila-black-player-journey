...
| Smoothing kernel sized in **metres**, with a tail cut on event surfaces | A fixed radius in grid cells | The 220x220 grid spans the normalised map, so one cell is 2.6 m on Grand Rift and 4.5 m on Lockdown -- a constant cell radius smeared a single kill 50-78 m depending on the map, so an area holding two kills looked like fighting spilling well past its edges. Radius is now derived from a real distance (25 m for events, 45 m for movement) and cells under 18% of the ceiling are not drawn, since at that level they hold only another cell's kernel tail. Measured reach beyond the nearest kill: 50-78 m before, 13-19 m after |
